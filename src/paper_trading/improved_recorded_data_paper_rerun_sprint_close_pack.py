"""
Improved Recorded-Data Paper Rerun Sprint Close Pack

Module EEEEE - paper/offline close only.

This pack closes Phase 7 by aggregating the improved recorded-data paper rerun
planning, preflight, and execution-control evidence.
...
REPORT_TYPE = "improved_recorded_data_paper_rerun_sprint_close_pack"
SAFE_BROKER_MODE = "broker_disabled"
SAFE_DATA_MODE = "recorded_data_only"
SAFE_ORDER_MODE = "real_orders_disabled"

SAFETY_NOTICE = (
    "Improved recorded-data paper rerun sprint close only. This pack does not run "
...
class Phase7CloseReport:
    ...
    accepted_for_future_safe_paper_rerun_runner_build: bool
    ...
    inputs: list[Phase7InputStatus]


INPUT_TITLES = {
    "planning": "Improved recorded-data paper rerun planning",
    "preflight": "Improved recorded-data paper rerun preflight",
    "execution_control": "Improved recorded-data paper rerun execution control",
}

ACCEPTED_FLAGS = {
    "planning": "accepted_for_future_improved_rerun_execution",
    "preflight": "accepted_for_future_improved_rerun_runner",
    "execution_control": "accepted_for_future_improved_paper_rerun",
}
...
def build_phase7_close_report(
    *,
    ...
    issues: list[dict[str, Any]] = []
    input_statuses: list[Phase7InputStatus] = []
        ...
        issues.append(
            {
                ...
                "severity": "fail",
                "message": "One or more Phase 7 inputs allowed live/real money, profit claim, backtest execution, or strategy logic change.",
            }
        )
    ...
    return Phase7CloseReport(
        ...
        phase_7_complete=phase_complete,
        accepted_for_future_safe_paper_rerun_runner_build=phase_complete,
        completed_total_after_module=109,
        phase_7_pending_after_module=0 if phase_complete else 1,
        next_recommended_phase="Phase 8 - Safe improved paper rerun runner build",
        next_recommended_step=(
            "Build a paper-only improved recorded-data rerun runner using the locked "
            "Phase 7 controls. The runner should execute recorded-data paper simulation "
            "only, keep broker/live/real-order gates disabled, and label all outputs "
            "as non-profitability evidence."
        ),
...
def _write_csv(path: Path, inputs: list[Phase7InputStatus]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _text_report(report: Phase7CloseReport) -> str:
    lines = [
        "HQE Improved Recorded-Data Paper Rerun Sprint Close Pack",
        "=" * 80,
        ...
        f"Phase 7 complete: {report.phase_7_complete}",
        (
            "Accepted for future safe paper rerun runner build: "
            f"{report.accepted_for_future_safe_paper_rerun_runner_build}"
        ),
...
def write_phase7_close_pack(
    report: Phase7CloseReport,
    output_dir: Path,
) -> dict[str, str]:
    output_dir.mkdir(parents=True, exist_ok=True)
    report_json = output_dir / f"{REPORT_TYPE}.json"
    report_txt = output_dir / f"{REPORT_TYPE}.txt"
    inputs_csv = output_dir / "improved_recorded_data_paper_rerun_sprint_close_inputs.csv"
    ...
    manifest_payload = {
        ...
        "phase_7_complete": report.phase_7_complete,
        "accepted_for_future_safe_paper_rerun_runner_build": report.accepted_for_future_safe_paper_rerun_runner_build,
...
def build_and_write_phase7_close_pack(
    *,
    input_paths: dict[str, Path] | None = None,
    output_dir: Path = DEFAULT_OUTPUT_DIR,
) -> tuple[Phase7CloseReport, dict[str, str]]:
    report = build_phase7_close_report(
        input_paths=input_paths,
        output_dir=output_dir,
    )
    outputs = write_phase7_close_pack(report, output_dir)
    return report, outputs


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Improved recorded-data paper rerun sprint close pack."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report, outputs = build_and_write_phase7_close_pack(
        output_dir=Path(args.output_dir),
    )

    print("HQE improved recorded-data paper rerun sprint close pack completed.")
    print(report.safety_notice)
    print(report.no_profitability_claim_notice)
    print(f"Status: {report.status}")
    print(f"Phase 7 complete: {report.phase_7_complete}")
    print(f"Accepted for future safe paper rerun runner build: {report.accepted_for_future_safe_paper_rerun_runner_build}")
