...
One-command evidence-only readiness gate for the adapter evidence layer:
1. Build adapter evidence bundle.
2. Run adapter evidence bundle acceptance.
3. Write final adapter evidence readiness report.
...
from src.paper_trading.recorded_data_paper_strategy_adapter_evidence_bundle import (
    build_and_write_adapter_evidence_bundle_report,
)
...
class AdapterEvidenceReadinessStageResult:
    stage: str
    status: str
...
@dataclass(frozen=True)
class AdapterEvidenceReadinessReport:
    generated_at_utc: str
    status: str
    ready_for_future_paper_strategy_adapter_evidence_release: bool
    output_directory: str
    ...
    allow_warnings: bool
    safety_notice: str
    stage_results: list[AdapterEvidenceReadinessStageResult]


def safety_notice() -> str:
    return (
        "Paper/simulation evidence only. This adapter evidence readiness gate "
...
def build_adapter_evidence_readiness_report(
    *,
...
) -> AdapterEvidenceReadinessReport:
    bundle_report, bundle_outputs = build_and_write_adapter_evidence_bundle_report(
        ...
        evidence_output_dir=adapter_evidence_bundle_output_dir,
    ...
    bundle_stage = AdapterEvidenceReadinessStageResult(
        stage="recorded_data_paper_strategy_adapter_evidence_bundle",
        status=bundle_report.status,
        output_directory=str(adapter_evidence_bundle_output_dir),
        output_files=_stringify_outputs(bundle_outputs),
        accepted=bundle_report.ready_for_future_adapter_evidence,
        summary={
            "ready_for_future_adapter_evidence": (
                bundle_report.ready_for_future_adapter_evidence
            ),
            ...
            "message": "No-execution adapter evidence bundle completed.",
        },
    )

    acceptance_report, acceptance_outputs = (
        build_and_write_adapter_evidence_bundle_acceptance_report(
            adapter_evidence_bundle_path=(
                bundle_outputs["paper_strategy_adapter_evidence_bundle_json"]
            ),
            output_dir=adapter_evidence_bundle_acceptance_output_dir,
            min_stages=min_stages,
            allow_warnings=allow_warnings,
        )
    )

    acceptance_stage = AdapterEvidenceReadinessStageResult(
        stage="recorded_data_paper_strategy_adapter_evidence_bundle_acceptance",
        status=acceptance_report.status,
        output_directory=str(adapter_evidence_bundle_acceptance_output_dir),
        ...
        summary={
            ...
            "message": "Adapter evidence bundle acceptance gate completed.",
        },
    )

    status = _overall_status(
        bundle_status=bundle_report.status,
        bundle_ready=bundle_report.ready_for_future_adapter_evidence,
        acceptance_status=acceptance_report.status,
        acceptance_accepted=acceptance_report.accepted,
    )

    return AdapterEvidenceReadinessReport(
        generated_at_utc=datetime.now(timezone.utc).isoformat(),
        status=status,
        ready_for_future_paper_strategy_adapter_evidence_release=status in {
            "pass",
            "warn",
        },
...
def write_adapter_evidence_readiness_report(
    report: AdapterEvidenceReadinessReport,
    output_dir: Path,
) -> dict[str, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)

    readiness_json = output_dir / "paper_strategy_adapter_evidence_readiness.json"
    readiness_txt = output_dir / "paper_strategy_adapter_evidence_readiness.txt"
    ...
    lines = [
        "HQE Recorded Data Paper Strategy Adapter Evidence Readiness Gate",
        "",
        ...
        (
            "Ready for future paper strategy adapter evidence release: "
            f"{report.ready_for_future_paper_strategy_adapter_evidence_release}"
        ),
    ...
    lines.extend(
        [
            ...
            f"- {manifest_json}",
            "",
            "This gate only checks structural adapter evidence readiness.",
    ...
    manifest = {
        "report_type": "recorded_data_paper_strategy_adapter_evidence_readiness",
        "generated_at_utc": report.generated_at_utc,
        "status": report.status,
        "ready_for_future_paper_strategy_adapter_evidence_release": (
            report.ready_for_future_paper_strategy_adapter_evidence_release
        ),
        ...
        "outputs": {
            "paper_strategy_adapter_evidence_readiness_json": str(readiness_json),
            "paper_strategy_adapter_evidence_readiness_txt": str(readiness_txt),
    ...
    return {
        "paper_strategy_adapter_evidence_readiness_json": readiness_json,
        "paper_strategy_adapter_evidence_readiness_txt": readiness_txt,
        "manifest_json": manifest_json,
    }


def build_and_write_adapter_evidence_readiness_report(
    *,
    plan_readiness_path: Path,
    replay_plan_path: Path,
    adapter_contract_output_dir: Path,
    ...
    allow_warnings: bool = False,
    max_requests: int | None = None,
) -> tuple[AdapterEvidenceReadinessReport, dict[str, Path]]:
    report = build_adapter_evidence_readiness_report(
        plan_readiness_path=plan_readiness_path,
        replay_plan_path=replay_plan_path,
        adapter_contract_output_dir=adapter_contract_output_dir,
        adapter_contract_acceptance_output_dir=adapter_contract_acceptance_output_dir,
    ...
    outputs = write_adapter_evidence_readiness_report(report, readiness_output_dir)
    return report, outputs


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run no-execution paper strategy adapter evidence readiness gate."
    )
    parser.add_argument(
        "--plan-readiness",
        default=(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report, outputs = build_and_write_adapter_evidence_readiness_report(
    ...
    print("HQE recorded data paper strategy adapter evidence readiness completed.")
    ...
    print(
        "Adapter evidence readiness report: "
        f"{outputs['paper_strategy_adapter_evidence_readiness_txt']}"
    )
