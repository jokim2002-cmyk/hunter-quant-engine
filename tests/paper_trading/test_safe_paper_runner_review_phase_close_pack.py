...
from src.paper_trading.safe_paper_runner_review_phase_close_pack import (
    build_and_write_phase10_close_pack,
    build_phase10_close_report,
)
...
def _write_criteria(path: Path, *, safe: bool = True, accepted: bool = True) -> None:
    ...
    if not safe:
        payload["strategy_logic_changed"] = True
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload), encoding="utf-8")


def test_phase10_close_accepts_safe_criteria_and_closes_phase10(tmp_path: Path) -> None:
    criteria = tmp_path / "criteria.json"
    _write_criteria(criteria)

    report = build_phase10_close_report(
        review_criteria_input_path=criteria,
        output_dir=tmp_path / "out",
    )

    assert report.status == "pass"
    assert report.review_criteria_found is True
    assert report.review_criteria_status == "pass"
    assert report.review_criteria_accepts_close is True
    assert report.phase_10_complete is True
    assert report.accepted_for_future_phase11_safe_paper_runner_execution_review is True
...
def test_phase10_close_writes_json_text_csv_and_manifest(tmp_path: Path) -> None:
    criteria = tmp_path / "criteria.json"
    ...
    assert manifest["report_type"] == "safe_paper_runner_review_phase_close_pack"
    assert manifest["phase_10_complete"] is True
    assert manifest["accepted_for_future_phase11_safe_paper_runner_execution_review"] is True
...
def test_phase10_close_rejects_unsafe_criteria_without_live_or_profit_claim(tmp_path: Path) -> None:
    criteria = tmp_path / "criteria.json"
    ...
    assert report.phase_10_complete is False
    assert report.accepted_for_future_phase11_safe_paper_runner_execution_review is False
    assert report.runner_execution_enabled is False
    assert report.ready_for_live_or_real_money is False
    assert report.profitability_claim_allowed is False
    assert any(issue["code"] == "unsafe_review_criteria_input_boundary" for issue in report.issues)
