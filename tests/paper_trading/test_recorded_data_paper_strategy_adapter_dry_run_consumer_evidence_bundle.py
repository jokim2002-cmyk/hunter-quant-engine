...
from src.paper_trading.recorded_data_paper_strategy_adapter_dry_run_consumer_evidence_bundle import (
    build_adapter_dry_run_consumer_evidence_bundle_report,
...
def _build(tmp_path, **kwargs):
    ...
    return build_adapter_dry_run_consumer_evidence_bundle_report(
        ...
        evidence_output_dir=tmp_path / "consumer_evidence_bundle",
        **kwargs,
    )


def test_safety_notice_preserves_consumer_evidence_bundle_boundary():
...
def test_consumer_evidence_bundle_passes_valid_inputs(tmp_path):
...
def test_consumer_evidence_bundle_fails_when_min_requests_not_met(tmp_path):
    report = _build(tmp_path, min_requests=2)

    assert report.status == "fail"
    assert report.ready_for_future_consumer_evidence is False
    assert report.stage_results[0].accepted is False


def test_consumer_evidence_bundle_fails_when_min_total_bars_not_met(tmp_path):
...
def test_consumer_evidence_bundle_blocks_warning_by_default(tmp_path):
    report = _build(
        tmp_path,
...
def test_consumer_evidence_bundle_allows_warning_when_requested(tmp_path):
    report = _build(
        tmp_path,
        readiness=_plan_readiness(status="warn", ready=True),
        allow_warnings=True,
...
def test_consumer_evidence_bundle_fails_invalid_plan_readiness(tmp_path):
    report = _build(
        tmp_path,
...
def test_consumer_evidence_bundle_fails_wrong_replay_plan_modes(tmp_path):
...
def test_consumer_evidence_bundle_fails_forbidden_replay_plan_fields(tmp_path):
    plan = _replay_plan()
    plan["pnl"] = 99
...
def test_build_and_write_consumer_evidence_bundle_outputs_safety_and_no_profit_claim(
    tmp_path,
):
    ...
    text_report = outputs[
        "paper_strategy_adapter_dry_run_consumer_evidence_bundle_txt"
    ].read_text(encoding="utf-8")
    manifest = json.loads(outputs["manifest_json"].read_text(encoding="utf-8"))

    assert report.status == "pass"
    assert outputs["paper_strategy_adapter_dry_run_consumer_evidence_bundle_json"].exists()
    assert outputs["paper_strategy_adapter_dry_run_consumer_evidence_bundle_txt"].exists()
...
def test_documentation_mentions_consumer_evidence_bundle_shortcut():
    doc_paths = [
        Path("docs/RECORDED_DATA_PAPER_STRATEGY_ADAPTER_DRY_RUN_CONSUMER_EVIDENCE_BUNDLE.md"),
        Path("README.md"),
        Path("ROADMAP.md"),
    ]
    ...
    assert "hqe_recorded_data_paper_strategy_adapter_dry_run_consumer_evidence_bundle.bat" in combined
