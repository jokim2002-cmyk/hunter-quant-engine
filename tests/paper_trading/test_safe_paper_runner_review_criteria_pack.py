...
from src.paper_trading.safe_paper_runner_review_criteria_pack import (
    build_and_write_review_criteria_pack,
    build_review_criteria_report,
)
...
def _write_readiness(path: Path, *, safe: bool = True, started: bool = True) -> None:
    payload = {
        "status": "pass",
        "phase10_started": started,
        "accepted_for_future_safe_paper_runner_review_module": True,
...
def test_review_criteria_accepts_safe_readiness_and_locks_criteria(tmp_path: Path) -> None:
    readiness = tmp_path / "readiness.json"
    _write_readiness(readiness)

    report = build_review_criteria_report(
        review_readiness_input_path=readiness,
        output_dir=tmp_path / "out",
    )

    assert report.status == "pass"
    assert report.review_readiness_found is True
    assert report.review_readiness_status == "pass"
    assert report.phase10_started is True
    assert report.readiness_accepts_review_module is True
    assert report.accepted_for_future_safe_paper_runner_review_close is True
    assert report.criterion_count == 6
    assert report.locked_criterion_count == 6
    assert report.criteria_mode == "criteria_only"
...
def test_review_criteria_writes_json_text_csv_and_manifest(tmp_path: Path) -> None:
    readiness = tmp_path / "readiness.json"
    ...
    assert manifest["report_type"] == "safe_paper_runner_review_criteria_pack"
    assert manifest["accepted_for_future_safe_paper_runner_review_close"] is True
    assert manifest["criteria_mode"] == "criteria_only"
...
def test_review_criteria_rejects_unsafe_readiness_without_enabling_runner(tmp_path: Path) -> None:
    readiness = tmp_path / "readiness.json"
    ...
    assert report.accepted_for_future_safe_paper_runner_review_close is False
    assert report.runner_execution_enabled is False
    assert report.ready_for_live_or_real_money is False
    assert report.profitability_claim_allowed is False
    assert any(issue["code"] == "unsafe_review_readiness_input_boundary" for issue in report.issues)
