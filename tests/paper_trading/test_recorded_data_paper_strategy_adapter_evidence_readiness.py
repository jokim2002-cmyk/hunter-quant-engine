...
from src.paper_trading.recorded_data_paper_strategy_adapter_evidence_readiness import (
    build_adapter_evidence_readiness_report,
...
def _build(tmp_path, **kwargs):
    ...
    return build_adapter_evidence_readiness_report(
        ...
        readiness_output_dir=tmp_path / "adapter_evidence_readiness",
        **kwargs,
    )


def test_safety_notice_preserves_adapter_evidence_readiness_boundary():
...
def test_adapter_evidence_readiness_passes_valid_inputs(tmp_path):
    report = _build(tmp_path)

    assert report.status == "pass"
    assert report.ready_for_future_paper_strategy_adapter_evidence_release is True
...
def test_adapter_evidence_readiness_fails_when_min_requests_not_met(tmp_path):
    report = _build(tmp_path, min_requests=2)

    assert report.status == "fail"
    assert report.ready_for_future_paper_strategy_adapter_evidence_release is False
    assert report.stage_results[0].accepted is False


def test_adapter_evidence_readiness_fails_when_min_total_bars_not_met(tmp_path):
...
def test_adapter_evidence_readiness_blocks_warning_by_default(tmp_path):
    report = _build(
        tmp_path,
...
def test_adapter_evidence_readiness_allows_warning_when_requested(tmp_path):
    report = _build(
        tmp_path,
        readiness=_plan_readiness(status="warn", ready=True),
        allow_warnings=True,
...
def test_adapter_evidence_readiness_fails_invalid_plan_readiness(tmp_path):
    report = _build(
        tmp_path,
...
def test_adapter_evidence_readiness_fails_wrong_replay_plan_modes(tmp_path):
...
def test_adapter_evidence_readiness_fails_forbidden_replay_plan_fields(tmp_path):
    plan = _replay_plan()
    plan["pnl"] = 99
...
def test_build_and_write_adapter_evidence_readiness_outputs_safety_and_no_profit_claim(tmp_path):
    ...
    text_report = outputs["paper_strategy_adapter_evidence_readiness_txt"].read_text(
        encoding="utf-8"
    )
    manifest = json.loads(outputs["manifest_json"].read_text(encoding="utf-8"))

    assert outputs["paper_strategy_adapter_evidence_readiness_json"].exists()
    assert outputs["paper_strategy_adapter_evidence_readiness_txt"].exists()
    assert outputs["manifest_json"].exists()
    assert report.status == "pass"
    assert "not a profitability claim" in text_report
    assert manifest["ready_for_future_paper_strategy_adapter_evidence_release"] is True


def test_documentation_mentions_adapter_evidence_readiness_shortcut():
    doc_paths = [
        Path("docs/RECORDED_DATA_PAPER_STRATEGY_ADAPTER_EVIDENCE_READINESS.md"),
        Path("README.md"),
        Path("ROADMAP.md"),
    ]
    ...
    assert "hqe_recorded_data_paper_strategy_adapter_evidence_readiness.bat" in combined
