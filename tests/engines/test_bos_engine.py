...
def test_bos_engine_detects_bullish_bos_on_close_above_higher_high():
    candles = [
        make_candle(0, 100),
        make_candle(1, 105),
        make_candle(2, 111),
    ...
    result = engine.detect_bos(candles, [higher_high])

    assert len(result) == 1
    assert result[0].index == 2
    assert result[0].break_price == 111
    assert result[0].bos_type == BOSType.BULLISH
    assert result[0].broken_swing == higher_high


def test_bos_engine_detects_bullish_bos_on_close_above_lower_high():
    candles = [
        make_candle(0, 100),
        make_candle(1, 105),
        make_candle(2, 106),
    ...
    result = engine.detect_bos(candles, [lower_high])

    assert len(result) == 1
    assert result[0].bos_type == BOSType.BULLISH


def test_bos_engine_detects_bearish_bos_on_close_below_higher_low():
    candles = [
        make_candle(0, 100),
        make_candle(1, 95),
        make_candle(2, 89),
    ...
    result = engine.detect_bos(candles, [higher_low])

    assert len(result) == 1
    assert result[0].index == 2
    assert result[0].break_price == 89
    assert result[0].bos_type == BOSType.BEARISH
    assert result[0].broken_swing == higher_low


def test_bos_engine_detects_bearish_bos_on_close_below_lower_low():
    candles = [
        make_candle(0, 100),
        make_candle(1, 95),
        make_candle(2, 88),
    ...
    result = engine.detect_bos(candles, [lower_low])
...
def test_bos_engine_returns_empty_when_no_market_structure_points():
    candles = [
        make_candle(0, 100),
        make_candle(1, 101),
    ]
...
def test_bos_engine_creates_only_one_bos_for_same_structure_point():
    candles = [
        make_candle(0, 100),
        make_candle(1, 111),
        make_candle(2, 112),
